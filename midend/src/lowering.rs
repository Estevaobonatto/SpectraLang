...
impl ASTLowering {
    ...
    fn lower_statement(&mut self, stmt: &Statement, ir_func: &mut IRFunction) {
        match &stmt.kind {
            ...
            StatementKind::Break => {
                // TODO: Track loop exit blocks and branch to them
                // For now, create unreachable
                if let Some(current_block) = self.builder.get_current_block() {
                    if let Some(block) = ir_func.get_block_mut(current_block) {
            ...
            StatementKind::Continue => {
                // TODO: Track loop header blocks and branch to them
                // For now, create unreachable
                if let Some(current_block) = self.builder.get_current_block() {
                    if let Some(block) = ir_func.get_block_mut(current_block) {
                        block.set_terminator(Terminator::Unreachable);
    ...
    fn lower_expression(&mut self, expr: &Expression, ir_func: &mut IRFunction) -> Value {
        match &expr.kind {
            ...
            ExpressionKind::If { .. } => {
                // TODO: Implement if expression lowering with PHI nodes
                ir_func.next_value()
            }
            ExpressionKind::Unless { .. } => {
                // TODO: Implement unless expression lowering
                ir_func.next_value()
            }
