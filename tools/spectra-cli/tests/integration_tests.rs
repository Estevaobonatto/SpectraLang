...
fn test_compile_simple_test() {
    ...
    assert!(source.contains("module test"), "Should contain module declaration");
}
...
fn test_compile_math_functions() {
    ...
    assert!(!source.is_empty(), "Source file should not be empty");
    assert!(source.contains("factorial"), "Should contain factorial function");
    assert!(source.contains("fibonacci"), "Should contain fibonacci function");
}
