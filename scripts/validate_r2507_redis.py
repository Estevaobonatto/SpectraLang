...
def run(command: list[str], env: dict[str, str]) -> dict:
    ...
    return {"command": command, "exit_code": process.returncode, "stdout_tail": process.stdout[-2000:], "stderr_tail": process.stderr[-2000:]}
...
def independent_probe(url: str) -> dict:
    ...
    return {"ping": ping.startswith(b"+PONG"), "set": set_result.startswith(b"+OK"), "get": b"ok" in get_result, "delete": b":1" in delete_result}
...
def main() -> int:
    ...
    if not url:
        ...
        try:
            ...
            if not all(report["connection"]["independent_probe"].values()): report["failures"].append("independent Redis RESP probe failed")
        except Exception as error:
            report["failures"].append(f"Redis endpoint unavailable: {error}")
        env = os.environ.copy(); env["SPECTRA_REDIS_URL"] = url
        tests = run(["cargo", "test", "-p", "spectra-db", "--test", "redis_integration", "--", "--test-threads=1"], env)
