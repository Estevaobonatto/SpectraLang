...
- R-2903 owns native debug emission. Windows AOT debug builds attach compiler-owned CodeView records and request linker-produced PDBs; Unix builds generate DWARF v4 DIEs and line programs through `gimli` and attach them to the native object when a Unix target/toolchain is available. The `.spectra-debug.json` file remains supplementary metadata and is never accepted as a replacement for the native artifact. Structural parsing is mandatory in the Phase 29 gate; interactive debugger smoke must execute when a usable debugger is available and is not satisfied by tool discovery alone.
