...
- [x] Verify CLI and compiler flags gate experimental syntax behind feature toggles when needed _(no active syntax gates remain after `switch`, `if not`, `do-while`, and `loop` promotion; future gates must update the maturity policy and CLI list together)_
