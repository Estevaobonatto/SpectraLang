...
- A stable C ABI header and C sample are provided. Local validation uses LLVM `clang` against the release import library generated by Cargo.
...
Compile and run with LLVM `clang`:

```powershell
cargo build -p spectra-interop --release
& "C:\Program Files\LLVM\bin\clang.exe" `
  -I tools\spectra-interop\include `
  tools\spectra-interop\examples\c_ffi_sample.c `
  target\release\spectra_interop.dll.lib `
  -o target\release\c_ffi_sample.exe
